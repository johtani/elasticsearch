...
In 5.0, Synonym Token Filter tokenize synonyms with whatever
...
If `tokenizer` is set, this token filter same behavior in 1.x.
If you need same behavior in 1.x without `tokenizer` parameter,
